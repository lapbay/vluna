...
type AdminGateClient = {
  authorize: (body: Record<string, unknown>, context?: Record<string, unknown>) => Promise<unknown>
  commit: (body: Record<string, unknown>, context?: Record<string, unknown>) => Promise<unknown>
  cancel: (body: Record<string, unknown>, context?: Record<string, unknown>) => Promise<unknown>
  revokeSeat: (body: Record<string, unknown>, context?: Record<string, unknown>) => Promise<unknown>
}

class AdminGateStatusError extends Error {
  readonly status: number

  constructor(message: string, status: number) {
    super(message)
    this.name = 'AdminGateStatusError'
    this.status = status
  }
}

type AdminPlaneConfig = {
  baseUrl: string
  baseRealm: string
  serviceKeyId: string
...
const runtimeReconcileCommand: Command = {
  name: 'runtime.reconcile',
  summary: 'Reconcile runtime realm status with admin-plane realm slot seats.',
  requiresMigrator: false,
  async run(ctx) {
    const args = parseArgs(ctx.argv)
    const featureCode = requireStringFlag(args, 'feature-code')
    const realmId = (getStringFlag(args, 'realm-id') || '').trim() || undefined
    const orgId = (getStringFlag(args, 'org-id') || '').trim() || undefined
    const limitRaw = getStringFlag(args, 'limit')
    const dryRun = Boolean(args.flags['dry-run'])
    const limit = limitRaw ? Math.max(1, Math.min(Number(limitRaw), 2000)) : 500
    if (!Number.isFinite(limit) || limit <= 0) {
      throw new Error('Invalid --limit (must be a positive number)')
    }

    const adminConfig = resolveAdminPlaneConfig()
    const adminGate = await createAdminGateClient(adminConfig)
    const rows = await listRuntimeRealmRows({ realmId, orgId, limit })
    const result = {
      ok: true,
      dryRun,
      scanned: rows.length,
      ...
      skippedDryRun: 0,
      failed: 0,
      errors: [] as Array<{ realmId: string; orgId: string; stage: string; message: string }>,
    }

    for (const row of rows) {
      const requestId = `runtime.realm_slot.reconcile.${randomUUID()}`
      ...
      if (shouldBeActive) {
        let leaseToken = ''
        try {
          const auth = await adminGate.authorize(
            {
              feature_code: featureCode,
              seat_id: row.realmId,
            },
            {
              principalId: row.orgId,
              requestId,
            },
          )
          assertAdminGateSuccess(auth, 'authorize')
          leaseToken = extractLeaseToken(auth)
          if (!leaseToken) {
            throw new Error('missing lease_token from admin authorize response')
          }

          const commit = await adminGate.commit(
            {
              feature_code: featureCode,
          ...
          assertAdminGateSuccess(commit, 'commit')
          result.activeEnsured += 1
        } catch (error) {
          result.failed += 1
          result.errors.push({
            realmId: row.realmId,
            orgId: row.orgId,
            stage: 'authorize_commit',
            message: error instanceof Error ? error.message : String(error),
          })
          if (leaseToken) {
            try {
              const cancel = await adminGate.cancel(
                { lease_token: leaseToken },
                {
                  principalId: row.orgId,
                  requestId,
              ...
              assertAdminGateSuccess(cancel, 'cancel')
            } catch {}
          }
        }
        continue
      ...
      try {
        const revoke = await adminGate.revokeSeat(
          {
            feature_code: featureCode,
            seat_id: row.realmId,
          },
        ...
        if (extractTransportStatus(revoke) === 404) {
          continue
        }
        assertAdminGateSuccess(revoke, 'revokeSeat')
        result.revokedEnsured += 1
      } catch (error) {
...
const commands: Record<string, Command> = {
  ...
  'runtime.reconcile': runtimeReconcileCommand,
...
async function listRuntimeRealmRows(input: {
  realmId?: string
  orgId?: string
  limit: number
}): Promise<RuntimeRealmRow[]> {
  const rows = await db()
    .selectFrom('realms as r')
    .innerJoin('cloud_realm_members as m', 'm.realm_id', 'r.realm_id')
    .select(['r.realm_id as realmId', 'r.status as status', 'm.subject_id as orgId'])
    .where('m.kind', '=', 'organization')
    .$if(!!input.realmId, (qb) => qb.where('r.realm_id', '=', input.realmId as string))
    .$if(!!input.orgId, (qb) => qb.where('m.subject_id', '=', input.orgId as string))
    .where('r.status', 'in', ['active', 'suspended', 'deleted'])
    .orderBy('r.realm_id asc')
    .limit(input.limit)
    .execute()

  return rows.map((row) => ({
    realmId: String(row.realmId),
    status: row.status as RuntimeRealmRow['status'],
    orgId: String(row.orgId),
  }))
}

function resolveAdminPlaneConfig(env: NodeJS.ProcessEnv = process.env): AdminPlaneConfig {
  const baseUrl = env.VLUNA_ADMIN_BASE_URL?.trim()
  const keyId = env.VLUNA_ADMIN_SERVICE_KEY_ID?.trim()
  const secret = env.VLUNA_ADMIN_SERVICE_KEY_SECRET?.trim()
  const baseRealm = env.VLUNA_ADMIN_BASE_REALM?.trim()
  const legacyServiceKey = env.VLUNA_ADMIN_SERVICE_KEY?.trim()
  const timeoutRaw = env.VLUNA_ADMIN_TIMEOUT_MS?.trim()
  const timeoutMs = timeoutRaw ? Number(timeoutRaw) : 3000

  let resolvedKeyId = keyId
  let resolvedSecret = secret
  if ((!resolvedKeyId || !resolvedSecret) && legacyServiceKey) {
    const [legacyId, legacySecret] = legacyServiceKey.split(':', 2)
    if (legacyId && legacySecret) {
      resolvedKeyId = legacyId
      resolvedSecret = legacySecret
    }
  }

  if (!baseUrl || !baseRealm || !resolvedKeyId || !resolvedSecret) {
    throw new Error(
      'Missing admin plane config. Required: VLUNA_ADMIN_BASE_URL, VLUNA_ADMIN_BASE_REALM, VLUNA_ADMIN_SERVICE_KEY_ID, VLUNA_ADMIN_SERVICE_KEY_SECRET',
    )
  }
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    throw new Error('Invalid VLUNA_ADMIN_TIMEOUT_MS (must be positive number)')
  }
  return {
    baseUrl,
    baseRealm,
    serviceKeyId: resolvedKeyId,
    serviceKeySecret: resolvedSecret,
    timeoutMs,
  }
}

async function createAdminGateClient(config: AdminPlaneConfig): Promise<AdminGateClient> {
  const dynamicImport = new Function('specifier', 'return import(specifier)') as (specifier: string) => Promise<unknown>
  let moduleValue: unknown
  try {
    moduleValue = await dynamicImport('@vlunaai/sdk')
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    throw new Error(`@vlunaai/sdk is not available: ${message}`)
  }
  const createServiceClient = (moduleValue as { createServiceClient?: unknown }).createServiceClient
  if (typeof createServiceClient !== 'function') {
    throw new Error('createServiceClient is missing from @vlunaai/sdk')
  }
  const client = (createServiceClient as (options: unknown) => { s2s?: { gate?: unknown } })({
    config: {
      baseUrl: config.baseUrl,
      realmId: config.baseRealm,
      timeoutMs: config.timeoutMs,
    },
    errorMode: 'result',
    serviceKey: {
      keyId: config.serviceKeyId,
      secret: config.serviceKeySecret,
    },
  })
  const gate = client?.s2s?.gate as AdminGateClient | undefined
  if (!gate) {
    throw new Error('Invalid @vlunaai/sdk service client: missing s2s.gate')
  }
  return gate
}

function extractLeaseToken(response: unknown): string {
  if (!response || typeof response !== 'object') return ''
  const responseRecord = response as Record<string, unknown>
  const maybeData = responseRecord.data
  if (maybeData && typeof maybeData === 'object') {
    const envelope = maybeData as Record<string, unknown>
    const nestedData = envelope.data
    if (nestedData && typeof nestedData === 'object') {
      const candidate = (nestedData as Record<string, unknown>).lease_token
      if (typeof candidate === 'string' && candidate.length > 0) {
        return candidate
      }
    }
  }
  const direct = responseRecord.lease_token
...
function isStatusError(error: unknown, expectedStatus: number): boolean {
  if (error instanceof AdminGateStatusError) {
...
function assertAdminGateSuccess(response: unknown, stage: string): void {
  const status = extractTransportStatus(response)
  if (status === undefined || (status >= 200 && status < 300)) {
    return
  }
  const body = extractTransportBody(response)
  const bodyMessage = isRecord(body) && typeof body.message === 'string' ? body.message : ''
  const message = bodyMessage ? `${stage} failed: ${bodyMessage}` : `${stage} failed (status=${status})`
  throw new AdminGateStatusError(message, status)
}

function extractTransportStatus(response: unknown): number | undefined {
  if (!isRecord(response)) return undefined
  const status = response.status
  return typeof status === 'number' && Number.isFinite(status) ? status : undefined
}

function extractTransportBody(response: unknown): unknown {
  if (!isRecord(response)) return undefined
